 ...
 * the new 知识图谱 tab and that switching renders one of the
 * KG view's known states (loading / empty / unavailable / kg-view).
 *
...
test.describe("Memory page — 知识图谱 graph tab (UX-4 Slice 3.2)", () => {
	test("memory page shows both tabs and graph tab is reachable", async ({ page }) => {
		await page.goto("/memory");
		// Both tabs render — even when memory data is empty/unavailable.
		...
		await expect(page.getByTestId("memory-tab-list")).toHaveAttribute(
			"aria-selected",
			"true",
		);
		await expect(page.getByTestId("memory-tab-graph")).toHaveAttribute(
			"aria-selected",
			"false",
		);
	});
