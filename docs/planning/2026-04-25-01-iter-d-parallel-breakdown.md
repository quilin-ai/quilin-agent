...
| 25-02 cleanup sweep（Iter D 收口后）| `kg.py` (实证 `wc -l` = 530) / `retriever.py` (535) 拆分；S8 文档收口（默认跳过，仅在用户要求"完整冻结"时做 ADR-007 反链）；`store.py` 已 §16.1 拆完降至 491，不列入本 sweep | ⏳ 待启动 | — | Iter D 主轴 ✅ 收口，sweep 启动条件已达；规范源 `docs/planning/2026-04-25-02-c-m-cleanup-sweep.md` |
