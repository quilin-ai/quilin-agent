...
| 95% 覆盖率门槛（用户 2026-04-25 硬约束，凌驾 common/testing.md 80% 默认）| `@vitest/coverage-v8` + `pytest-cov`；vitest config thresholds 95；pyproject `cov-fail-under=95`；TS 补测试（Branches 79.88% → 95.03%）+ Python 补测试（TOTAL 91% → 95.25%） | ✅ 完成 | Python `f33b509` + TS `572bf96` | TS：73 files / 717 passed；Statements 96.92% / Branches 95.03% / Functions 96.57% / Lines 97.15%。Python：187 passed；TOTAL 95.25%。`just test-all` 三语言全过；AMB 100k recall p95 0.249ms（仅 recall gate） |
| R2 复核（Planck subagent）+ R2 fix pass | 独立第二个 subagent 复核 R1 全部 finding + 95% 覆盖率 + plan §17；fix pass 修 R2 三 finding | ✅ 完成 | `a09e940` + `572bf96`（fix）；R2 报告随 fix 一并 land | R2 初轮：BLOCKING 0 / HIGH 1 / MEDIUM 1 / LOW 1；HIGH-1（shell_exec ENOENT 真 bug）`a09e940` 修；MEDIUM-1（AMB 文档归因）+ LOW-1（frontmatter skipIf）`572bf96` 修；R2 follow-up：全 0 finding pass |
| Iter D 主轴最终收口 | 全部第一/二/三轮 + R1 + R2 全部 closed；TS/Python 95% 覆盖率；just test-all 三语言全绿；AMB recall gate；plan §17 残余归属仍稳 | ✅ 收口 | 全部 14 commits `56b7a46` → `572bf96` | TS 717 + Python 187 + Rust 1；biome/ruff/tsc/cargo 全 0；coverage TS Branches 95.03% / Python TOTAL 95.25%；AMB 100k p95 0.249ms < 300ms；R1/R2 finding 全 closed；plan §17 残余归属（M1.1 / LongMemEval / Arm L gate / 25-02 sweep）仍稳 |
| 25-02 cleanup sweep（Iter D 收口后）| `kg.py` 530 → 208 + `kg_query.py` 179 + `kg_validation.py` 208；`retriever.py` 535 → 208 + `retriever_bm25.py` 240 + `retriever_vector.py` 50 + `retriever_kg.py` 76；S8 文档收口默认跳过；`store.py` 491 不列 | ✅ 完成 | `1023ab5` | 主线（kg）+ Arendt worker（retriever）并行；ruff clean；`pytest -q` = 187 passed；coverage TOTAL 95.28% ≥ 95%；AMB 100k p95 0.286ms ≤ 300ms；`just test-all` TS 717 + Python 187 + Rust 1；public API 签名 diff 空；`MemoryClient.recall` / `MemoryStore.search` re-export 维持兼容 |
