...
| 组件 | 文件路径 | 接口 | 状态 |
|------|---------|------|------|
| LLMClient Protocol | `quilin/core/llm.py` | `chat() / chat_stream() / count_tokens()` | 待实现 |
| ThinkingMode Enum | `quilin/core/llm.py` | `OFF / INTERLEAVED / PRESERVED` | 待实现 |
| InferenceConfig | `quilin/core/llm.py` | frozen dataclass，预设 QUICK/STANDARD/DEEP | 待实现 |
| LLMCallbackHandler | `quilin/core/llm.py` | Protocol：on_llm_start/end/error | 待实现 |
| LiteLLMClient | `quilin/core/llm.py` | LLMClient 的具体实现 | 待实现 |
| Message / ToolCall / LLMResponse | `quilin/core/messages.py` | 已定义，LLMClient 直接使用 | 已完成 |
