...
`soul.md` is the project-level agent identity file defined by
[ADR-007](../../adr/adr-007-identity-files.md). M2.6 freezes the schema and
adds a read-only validator only; M2.7 static loading and any write path are out
...
patches and reviewed through the WriteAuthority boundary described in ADR-007.
