...
> **实现状态 / Implementation Status（2026-05-18 校准 / 2026-05-18 calibration）**
> - ✅ **Hot reload for MCP servers / SKILL.md / `~/.claude.json` (2026-05-15)**: `apps/web/lib/tools-loader.ts` 自动 watch providers 源码目录 + `~/.claude/skills` + `~/.claude.json`，编辑后自动 `invalidateToolsCatalog()` + `MCPRegistry.disconnectAll()`，下一次 chat 请求重建 catalog。带 mid-flight defer 计数器（`markToolCallInFlight()` / `markToolCallDone()`）：if a `shell_exec` is in flight, invalidate waits until it completes 避免 stdio 子进程被中途 disconnect 导致 promise 孤儿。env vars: `QUILIN_MCP_HOT_RELOAD=off` / `QUILIN_SKILL_HOT_RELOAD=off` 可关。Commits: `f2d15c6`, `fa6a343`, `ee2b802`, `5b001ed`, `f06d5ad`。
> - ✅ **Hot reload watchers fully shipped (2026-05-15)**: MCP `.py` source dirs + SKILL.md + `~/.claude.json` all auto-invalidate the tool catalog with mid-flight defer counter, so a `shell_exec` waiting on stdio is never orphaned by reload. Env vars to disable.
> - ✅ **Interaction primitives — 4 LLM-callable tools (2026-05-15)**: `ask_user_question` (LLM ↔ user Q&A), `request_approval` Path A (LLM-self gated), `narrate_aside` (italic side-channel narration), `wrapToolWithApproval` Path B server-side gate (already wrapping `shell_exec`, per-session whitelist where high/critical are never eligible). Spec: [`../07-safety-guardrails/interaction-primitives-spec.md`](../07-safety-guardrails/interaction-primitives-spec.md) (marked ✅). Commits: `5ec2192`, `44edf95`, `7c48bc4`, `74e9f1e`, `43e5f76`, `9b605e6`, `83b81d5`, `b7c0f02`, `f06d5ad`.
> - ✅ **SandboxRouter Done (QUI-62 closed 2026-05-18)**: `packages/agent-core/src/tools/sandbox*.ts` 共 2361 LOC + 59 tests, 6/6 acceptance 全覆盖 (filesystem mounts / output dirs / network policy / resource limits / snapshots / structured failure output). 见 [QUI-62 close-out comment](https://linear.app/quilin-agent/issue/QUI-62).
> - ✅ **MCP Stage 2 — Prompts + Elicitation client (QUI-166, 2026-05-18)**: `packages/agent-core/src/mcp/{client,prompts-client,elicitation-handler}.ts`。Quilin 现在可以消费第三方 MCP server 的 `prompts/list` + `prompts/get`，并处理 server-initiated elicitation。返回路径有 `scanExternalContext` 防 prompt injection；elicitation URL 白名单 frozen `['https:', 'http:']` 防 cast 篡改；`ThreatMatch.matchedText` 默认 sha256-16 fingerprint 不泄漏攻击原文（opt-in `includeMatchedText`）；`requestedSchema` 有 `validateSchemaBounds(maxDepth=8, maxKeys=100, maxStringLen=4000)`。Commits: `46bd315` + `15cd822` + `3625e9c`.
> - ✅ **Quilin-as-server Stage 3 stdio skeleton (QUI-171, 2026-05-18)**: `packages/agent-core/src/mcp/server/{quilin-mcp-server,exposed-tools,exposed-resources}.ts`。把 Quilin 变 MCP server 让 Claude Desktop / Cursor / Goose 等 client 消费。暴露白名单 4 工具（`memory_recall` / `memory_save` / `skill_search` / `web_fetch`）+ 3 资源（`quilin://profile/skills/recent-sessions`）。每个 `CallTool` 走 zod 入参验证（每字段都有 length/format cap），`web_fetch` 只允许 `http:` / `https:` + 拒绝 URL userinfo（和内置 web-fetch QUI-129 对齐）。完整连接状态机经 8 轮 cross-review：connect/close/onclose interleaving、failed transport cleanup、pending connect identity、close-during-connect 取消语义、旧 pending attempt settle 不覆盖新 retry。Peer 可控错误回显全清洗 C0+C1 控制字符（含 CSI U+009B + NEL U+0085）。**Stage 3 不开 HTTP listener**；Stage 4-5 + A2A federation 是后续 issue。Commits: `d2ec82a` + `83100d5` + `d41bc0b` + `51272b0` + `948e8e0`. Design doc: `docs/research/2026-05-18-quilin-as-server/README.md`.
>
> **2026-05-08 calibration（保留作历史）／ Preserved as historical baseline:**
> - ✅ **Implemented**: `packages/agent-core/src/tools/` includes ToolRegistry / Router, MCP client stdio spawn allowlist, schema converter, tool sanitizer/metadata, builtin `shell_exec` / file tools / `web_fetch` / `skill_search` / `skill_view` / `skill_manage`, plus WriteAuthority and SSRF guard wiring.
> - ✅ **已实现**：`packages/agent-core/src/tools/` 已包含 ToolRegistry / Router、MCP client stdio spawn allowlist、schema converter、tool sanitizer/metadata、builtin `shell_exec` / file tools / `web_fetch` / `skill_search` / `skill_view` / `skill_manage`，以及 WriteAuthority / SSRF guard 等安全接线。
> - ✅ **web_fetch upgraded to Claude Code parity (QUI-129)**: Turndown HTML→Markdown lazy singleton, optional `prompt` parameter that pipes the markdown through a sub-LLM extractor (`LLMClient` injected), URL LRU cache (15 min TTL / 50 MB budget by default), same-host redirect default with structured cross-host redirect responses, body cap raised 16 KB → 100 KB, response cap 5 MB → 10 MB, timeout 30 s → 60 s, redirects 3 → 10. Source: `packages/agent-core/src/tools/builtin/web-fetch{,-cache,-extract}.ts`.
> - ✅ **web_fetch 升级到 Claude Code 水位（QUI-129）**：HTML→Markdown 走 Turndown 懒加载单例，新增可选 `prompt` 参数把 markdown 喂进子 LLM 抽取器（注入 `LLMClient`），URL LRU 缓存（默认 15 分钟 TTL / 50 MB 预算），默认同源 redirect 自动跟随，跨域 redirect 返回结构化提示由上层决策，正文上限 16 KB → 100 KB，响应上限 5 MB → 10 MB，超时 30 s → 60 s，redirects 3 → 10。源文件：`packages/agent-core/src/tools/builtin/web-fetch{,-cache,-extract}.ts`。
> - ✅ **web_browse landed via Stagehand + Patchright (QUI-130)**: new `web_browse` tool wraps `@browserbasehq/stagehand` `act` / `observe` / `extract` LLM primitives over a headless browser, defaulting to Patchright's stealth Chromium runtime. The tool factory accepts an injectable `stagehandFactory` for tests, an optional `cdpUrl` for connecting to externally-launched browsers, and a `WriteAuthority` gate that ASKs for confirmation before any `act` mutation. `extract` / `observe` route as read; `act` is medium-risk and goes through the gate. Source: `packages/agent-core/src/tools/builtin/web-browse.ts`.
> - ✅ **web_browse 通过 Stagehand + Patchright 落地（QUI-130）**：新建 `web_browse` 工具封装 `@browserbasehq/stagehand` 的 `act` / `observe` / `extract` LLM 原语，运行时默认走 Patchright 的反爬 Chromium。工厂支持注入 `stagehandFactory` 便于测试、`cdpUrl` 连接外部启动的浏览器，并通过 `WriteAuthority` gate 在 `act` 模式下显式确认；`extract` / `observe` 走 read，`act` 走 medium 风险并经 gate 把关。源文件：`packages/agent-core/src/tools/builtin/web-browse.ts`。
> - ✅ **quilin-web Python provider via MCP (QUI-131)**: `providers/web/` ships a Crawl4AI-backed MCP server exposing `web_extract(url, query?)` (URL → cleaned RAG-ready markdown) and `web_crawl(url, depth, max_pages)` (shallow same-host crawl, depth ≤ 3, pages ≤ 50). The crawler is injected via a `crawler_factory` so tests run fully mocked (no Playwright, no Chromium download). Crawl4AI itself is an opt-in extra (`uv sync --extra crawler`). Source: `providers/web/`. Run: `just dev-web`.
> - ✅ **quilin-web Python provider 通过 MCP 接入（QUI-131）**：`providers/web/` 是基于 Crawl4AI 的 MCP server，暴露 `web_extract(url, query?)`（URL → 清洗过的 RAG 友好 markdown）和 `web_crawl(url, depth, max_pages)`（浅层同源爬取,depth ≤ 3,pages ≤ 50)。crawler 通过 `crawler_factory` 注入，测试 100% mock(不跑 Playwright、不下 Chromium)。Crawl4AI 是可选 extra(`uv sync --extra crawler`)。源文件：`providers/web/`。启动：`just dev-web`。
> - ✅ **E2E 验证 / E2E verified (2026-05-08)**: Phase 1 + 2 + 3 三连发已做端到端验收 — 25 用例 / 24 PASS / 1 FAIL（已修）。Phase 1 web_fetch 10/10 PASS（cache+prompt 短路 bug 在 session 内已修）；Phase 2 web_browse 8/9 PASS（[QUI-133](https://linear.app/quilin-agent/issue/QUI-133) 跟踪 userinfo URL 防御缺失）；Phase 3 quilin-web 6/6 PASS（[QUI-134](https://linear.app/quilin-agent/issue/QUI-134) 跟踪 Crawl4AI stdout 污染 MCP stdio）。详见 [`e2e-2026-05-08-web-tools.md`](./e2e-2026-05-08-web-tools.md) umbrella + 三份子报告。 / End-to-end verified for Phase 1 + 2 + 3: 25 cases / 24 PASS / 1 FAIL (fixed). Phase 1 `web_fetch` 10/10 PASS (cache+prompt short-circuit bug fixed in-session); Phase 2 `web_browse` 8/9 PASS (QUI-133 tracks missing userinfo guard); Phase 3 `quilin-web` 6/6 PASS (QUI-134 tracks Crawl4AI stdout pollution). See umbrella [`e2e-2026-05-08-web-tools.md`](./e2e-2026-05-08-web-tools.md) plus three sub-reports.
> - ✅ **quilin-web wired into REPL via MCPRegistry (QUI-142 + 2026-05-09 follow-up)**: `loader.ts` 的 `createDefaultCapabilitiesConfig` 在 `providers/{memory,web}/` 存在时自动把 `quilin-mem` 和 `quilin-web` 写进默认 `mcpServers` config；REPL 启动后 `MCPRegistry` 通过 `StdioClientTransport` 自动 spawn 这两个 server 并连接 stdio。早期 QUI-142 commit 同时通过 `mcp-launcher.ts` + MCPRegistry 双 spawn（4 个 Python 子进程而非 2 个），2026-05-09 修复后 `mcp-launcher.ts` 完全删除（QUI-144 S-2）；现在 spawn 只走 MCPRegistry 一条路径。REPL 对话直接说"爬一下文档站"命中 `web_crawl`，不再退化到多次 `web_fetch` 串行。`just dev` 仍可用，但**强烈推荐 `just dev-once`**（不带 `--watch`）做评估 — `--watch` 在并行编辑器/agent 改源码时会触发 in-process reload，叠加每次再 spawn 一对 MCP server 形成 Python 孤儿堆积。/ `loader.ts`'s `createDefaultCapabilitiesConfig` writes `quilin-mem` and `quilin-web` into the default `mcpServers` config whenever `providers/{memory,web}/` exists; on REPL startup `MCPRegistry` uses `StdioClientTransport` to spawn and connect the two servers. The original QUI-142 commit double-spawned via both `mcp-launcher.ts` and MCPRegistry (4 Python children instead of 2); the 2026-05-09 follow-up removed mcp-launcher calls and the 2026-05-09 cleanup (QUI-144 S-2) deleted the now-dead module entirely; spawn is now a single path through MCPRegistry. REPL conversations like "shallow-crawl this docs site" hit `web_crawl` directly. `just dev` still works, but **prefer `just dev-once`** (no `--watch`) for evaluation — concurrent edits to imported source files trigger in-process reload + an extra MCP spawn each, leaking Python orphans.
> - ✅ **Reactive-execution 路线图同期落档 / Reactive-execution roadmap filed in parallel**: 这次 E2E 触发了一整套 harness engineering 路线图 — 主索引 [`../00-core-loop/intelligence-roadmap.md`](../00-core-loop/intelligence-roadmap.md)，子 spec 为 [`../00-core-loop/reactive-execution.md`](../00-core-loop/reactive-execution.md)（Iter L）、[`../00-core-loop/eval-driven-development.md`](../00-core-loop/eval-driven-development.md)（Iter L+0，gate）、[`./tool-taste.md`](./tool-taste.md)（Iter L+1）、[`../02-context/auto-context-curation.md`](../02-context/auto-context-curation.md)（Iter L+2）。Linear: QUI-132 / 135 / 136 / 137 trackers + QUI-138 / 139 cross-cutting issues. / This E2E spawned a full harness-engineering roadmap — master index plus four sub-specs (Iter L / L+0 / L+1 / L+2). Linear: QUI-132 / 135 / 136 / 137 trackers + QUI-138 / 139 cross-cutting issues.
...
> **状态校准（2026-04-30）**：下表是早期 Python Harness 映射快照。当前工具实现路径在 `packages/agent-core/src/tools/`，DockerSandbox benchmark MVP 在 `benchmarks/src/sandbox/`；`quilin/tools/*.py` 不再是目标路径。

| 组件 | 文件路径 | 接口 | 状态 |
|------|---------|------|------|
| Tool Protocol | `packages/agent-core/src/tools/types.ts` | `Tool` Protocol + schema/result types | ✅ 当前实现 |
| ToolRegistry | `packages/agent-core/src/tools/registry.ts` | 注册/发现/过滤 | ✅ 当前实现 |
| ToolRouter | `packages/agent-core/src/tools/router.ts` | tool invocation routing | ✅ 当前实现 |
| MCPClient | `packages/agent-core/src/tools/mcp-client.ts` | 连接外部 MCP Server，stdio + allowlist/env guard | ✅ 当前实现 |
| MCPBus | （历史 Python Harness 概念） | 层间 MCP 通信总线 | superseded by TS MCP client + future mesh |
| BrowserProvider | `quilin/browser/provider.py` | 浏览器操作 + 多级路由 | future target |
| SessionManager | `quilin/browser/session.py` | 会话管理、Cookie、2FA | future target |
| SandboxExecutor | `benchmarks/src/sandbox/docker.ts` | DockerSandbox benchmark MVP；通用 sandbox family 未冻结 | partial |
...
> **状态校准（2026-04-30）**：本节表格仍可作为目标能力清单，但"当前状态"列保留早期快照。当前实现摘要以本文顶部状态块为准。
