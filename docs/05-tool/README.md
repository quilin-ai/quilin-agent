...
> **实现状态 / Implementation Status（2026-05-08 校准 / 2026-05-08 calibration）**
...
> - ✅ **quilin-web wired into REPL via MCPRegistry (QUI-142 + 2026-05-09 follow-up)**: `loader.ts` 的 `createDefaultCapabilitiesConfig` 在 `providers/{memory,web}/` 存在时自动把 `quilin-mem` 和 `quilin-web` 写进默认 `mcpServers` config；REPL 启动后 `MCPRegistry` 通过 `StdioClientTransport` 自动 spawn 这两个 server 并连接 stdio。**`mcp-launcher.ts` 不再被 REPL 调用**（保留为 dead module 备未来 service-mode 用），早期 QUI-142 commit 同时调 mcp-launcher + MCPRegistry 导致每个 server 双重 spawn（4 个 Python 子进程而非 2 个），2026-05-09 修复移除 mcp-launcher 调用后回归到一对。REPL 对话直接说"爬一下文档站"命中 `web_crawl`，不再退化到多次 `web_fetch` 串行。`just dev` 仍可用，但**强烈推荐 `just dev-once`**（不带 `--watch`）做评估 — `--watch` 在并行编辑器/agent 改源码时会触发 in-process reload，叠加每次再 spawn 一对 MCP server 形成 Python 孤儿堆积。/ `loader.ts`'s `createDefaultCapabilitiesConfig` writes `quilin-mem` and `quilin-web` into the default `mcpServers` config whenever `providers/{memory,web}/` exists; on REPL startup `MCPRegistry` uses `StdioClientTransport` to spawn and connect the two servers. **`mcp-launcher.ts` is no longer invoked from the REPL** (kept as a dead module for possible future service-mode use). The original QUI-142 commit called both mcp-launcher AND went through MCPRegistry, causing each server to be spawned twice (4 Python children instead of 2); the 2026-05-09 follow-up removed the mcp-launcher calls. REPL conversations like "shallow-crawl this docs site" hit `web_crawl` directly. `just dev` still works, but **prefer `just dev-once`** (no `--watch`) for evaluation — concurrent edits to imported source files trigger in-process reload + an extra MCP spawn each, leaking Python orphans.
