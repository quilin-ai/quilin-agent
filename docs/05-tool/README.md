...
> - ✅ **quilin-web wired into REPL (QUI-142, 2026-05-09)**: `mcp-launcher.ts` 现在通过共享的 `McpProviderRunner` helper 同时 spawn `quilin-mem` 和 `quilin-web`；`loader.ts` 默认配置自动包含 `quilin-web` 当 `providers/web/` 存在时。`just dev` 启动会同时打 `quilin-mem MCP started` + `quilin-web MCP started` 双行。REPL 里直接说"爬一下文档站"能命中 `web_crawl`，不再退化到多次 `web_fetch` 串行。/ `mcp-launcher.ts` now uses a shared `McpProviderRunner` helper to spawn both `quilin-mem` and `quilin-web`; `loader.ts` default config auto-includes `quilin-web` when `providers/web/` exists. `just dev` startup emits both `quilin-mem MCP started` and `quilin-web MCP started` log lines. REPL conversations like "shallow-crawl this docs site" hit `web_crawl` directly instead of degrading to a sequence of `web_fetch` calls.
