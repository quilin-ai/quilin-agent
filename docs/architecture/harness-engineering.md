...
> Quilin Agent 的顶层架构概念。本文档综合了 OpenAI、Anthropic、Martin Fowler、LangChain、Manus、Epsilla 等行业最前沿的 harness 工程实践（18 篇来源），将散落在 12 个激活工程领域中的 harness 相关设计统一为一个显式的一等架构理念。12-对话工程在 D-02（2026-04-17 ultra-review）中降级为 parked，等 benchmark 三大榜单有阶段性数据后再解冻。
...
Quilin 的整体架构就是一个 harness。E-T-C-S-L-V 六组件 + 12 个激活工程领域 + 1 parked 领域（12-对话工程）共同构成了这个 harness：
...
> 12-对话工程在 D-02 决策中降级为 parked（等 Iter F 解冻）；剩余 12 个领域为激活态，构成当前 harness 的全部前馈/反馈回路。

| 领域 | Harness 角色 | 控制类型 | 重要性 | 状态 |
|------|-------------|---------|--------|------|
| 01-LLM 接入 | 发动机接口（连接任意模型） | — | 基础 | Active |
| **02-上下文** | **整车底盘（prompt 组装 + token 预算 + KV-cache）** | **前馈** | **核心** | Active |
| **03-记忆** | **导航系统（跨会话知识积累）** | **前馈** | **核心** | Active |
| **04-规划** | **自动驾驶（策略选择 + 任务分解 + 重规划）** | **前馈** | **核心** | Active |
| **05-工具** | **手脚（与外界交互 + 自检能力）** | **前馈+反馈** | **核心** | Active |
| 06-多 Agent | 车队协作（多车协同 + 上下文防火墙） | 前馈+反馈 | 高级 | Active |
| **07-安全护栏** | **安全带 + ABS（约束即生产力）** | **前馈+反馈** | **核心** | Active |
| 08-可观测性 | 仪表盘（运行状态可见 + eval 驱动） | 反馈 | 重要 | Active |
| 09-部署运行时 | 车库和加油站（运行环境 + 熵管理） | — | 基础 | Active |
| **10-自进化** | **自动调校（harness 自己改自己 + 用户自助吸收）** | **前馈+反馈** | **核心差异化** | Active |
| 11-Agent Mesh | 车联网（与其他 agent 通信） | — | 内置能力 | Iter D |
| **13-技能工程** | **驾驶技巧手册（SKILL.md 目录 + 按需加载 → 喂给 02-上下文）** | **前馈** | **核心** | Active |
| 12-对话工程（parked） | 驾驶员体验（活人感 + 风格适配 + 关系建模） | 前馈 | 差异化 | **Parked → Iter F** |
