...
> **状态**：历史 threat-model v0.1（2026-04-18 首次定稿，2026-04-30 状态校准）
...
> **适用范围**：Iter A..D 时间窗的 Quilin Agent 运行时（单机为主；mesh runtime 已从 Iter D 延期到 Iter F）
> **覆盖方**：本地开发者、个人终端用户、CI runner、Codex/Claude 协作场景；**不**覆盖公有云多租户场景（Iter F+ 再起一份）
>
> **状态校准（2026-04-30）**：本文保留 v0.1 威胁枚举和设计依据。当前实现已补上 `web_fetch` SSRF guard、`WriteAuthority`、skills_guard、checkpoint reasoning secret stripping、CI hard gates；Agent Mesh mTLS / 远端 agent 威胁仍未进入 runtime。
...
### 3.6 Mesh / Remote Agent（Iter F+，预研）
...
|--------|---------------------|----------|:----:|
| PI-01..06 | Layer 1（输入） + ContextAssembler XML 隔离 | `packages/agent-core/src/context/injection-scanner.ts` + ContextAssembler / PromptBuilder | 🚧 规则型子集已落地，完整 XML 隔离未完成 |
...
| TH-04 | 13-skills validator | `packages/agent-core/src/skills/guard.ts` / `threat-patterns.ts` | ✅ B3b Phase 3 |
...
| ME-01..03 | 11-agent-mesh mTLS + 签名 | Iter F Day 0 spike | 💭 runtime 未启动 |
...
- **版本轨迹**：本文档放在 `docs/07-safety-guardrails/threat-model.md`，所有修订走 PR。
- **关联**：
  - 上游决策：[Core Loop](../00-core-loop/README.md) 和 [Safety Guardrails](./README.md)
