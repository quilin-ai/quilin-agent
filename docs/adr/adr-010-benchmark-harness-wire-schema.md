...
- `bfcl-v4`（Iter E3b partial AST slice）：`inputs.{question, messages, function_definitions}`；`expected.{ground_truth, expected_tool_calls, category, general_category}`；`metadata.{category, general_category, partial_eval: true, official_parity: false, source_commit}`；`scorer_type = "bfcl-v4-ast"`。`function_definitions` 保留官方 BFCL v4 raw function schema，scorer 读取其中 `parameters.properties[*].type/items.type` 执行 Java / JavaScript type conversion；category 仅通过 `metadata.category` / `metadata.general_category` 路由，不新增 top-level enum。
- `bfcl-v4`（Iter E3c1a multi-turn fixture slice）：`inputs.{question, turns}`；`expected.{possible_answer, initial_config, involved_classes, fixture_result?, category, general_category: "multi_turn"}`；`metadata.{category, general_category: "multi_turn", multi_turn: true, partial_eval: true, official_parity: false, stateful_eval: false, source_commit}`；`scorer_type = "bfcl-v4-multi-turn"`。`initial_config` 只能进入 scorer/runtime 侧，不得进入 agent prompt；fixture submission 输出官方 nested `result/<model>/multi_turn/BFCL_v4_<category>_result.json`，并在 manifest 标记 `stateful_eval=false`，防止与 E3c1b stateful runner 输出混淆。
