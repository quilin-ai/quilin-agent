...
>
> **Core Loop 对齐**：本 spec 的所有运行时实现遵循 [Core Loop](../00-core-loop/README.md)（核心循环 TS、无 LangGraph、packages/agent-core 结构）。
...
| **RouteLLM（成本/质量 tradeoff router）** | cost-sensitive routing，主 LLM call 之前决定用 weak 还是 strong | ⏸ 只在本地成本/质量实证阶段评估；Benchmark 已冻结，除非用户明确要求不得转成 Benchmark 工作 |
