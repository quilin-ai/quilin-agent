...
> | **M2 — 平台化 progressive disclosure + agentskills.io 客户端**（QUI-170）| ✅ Done（2026-05-18） | `packages/agent-core/src/skills/{progressive-disclosure,registry-client,url-guard}.ts`。Jaccard-like 关键词重叠（heading 3x weight）按 user intent 暴露 skill chunk + agentskills.io 注册表 client（search / pull / install with WriteAuthority CRITICAL gate）+ SSRF/OOM 守卫（URL 协议白名单 + 主机黑名单 + Content-Length + 流式 size + `requestTimeoutMs`）+ env opt-in `AGENTSKILLS_ALLOW_INSECURE=1`。Anthropic SKILL.md format 完全兼容 + ed25519 / sigstore signing **未做**（接口预留 `SkillSignatureVerifier`，留 follow-up）。Commits: `693a160` + `5edde63` + `a5eef44`. |
> | **M2+ — Skill signing / Anthropic SKILL.md format 兼容 / Background nudge / ToolSearch 延迟加载 / Skill 融合** | Linear [QUI-22](https://linear.app/quilin-agent/issue/QUI-22/13-skills-m2-platformization-toolsearch-and-background-nudge)、[QUI-98](https://linear.app/quilin-agent/issue/QUI-98/m4-实现-skill-融合将多个相似-skill-自动合并优化)（skill 融合/去重） | 依赖 10-self-evolution / Plugin 平台 / 工具数 >100。Skill signing 接口已在 M2 落地的 `registry-client.ts` 预留。 |
