...
```ts
// This enum enumerates *possible* optimizer kinds for design / forward
// compatibility, NOT a list of shipped optimizers. Post 2026-05-12 the
// only IMPLEMENTED kind is `gepa_textual_gradient` (the GEPA-only
// decision in docs/10-self-evolution/README.md §2.4 closed the picking
// decision). Other entries are speculative; adding a new optimizer
// requires landing actual code + a fresh decision-record, not just a
// new enum member.
