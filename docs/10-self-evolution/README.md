...
|---|---|---|
| `PromptRewrite` (baseline heuristic) | 82.0% | [75.1%, 87.3%] |
| DSPy + MIPROv2 (real, DummyLM) | 100.0% | [97.5%, 100.0%] |
| DSPy + GEPA (real, DummyLM) | 100.0% | [97.5%, 100.0%] |

Both DSPy arms regress to total failure (lift = **−22.0%** for both), landing in the **lift < 10%** bucket which would normally trigger Stage E. **However the regression is unsurprising and explainable**: `DummyLM` cycles deterministic but content-free responses, so the DSPy compile loops emit "optimized" prompts that lack the keyword overlap the replay scorer rewards. The `PromptRewrite` baseline beats DSPy here only because its per-category templates are hand-written to match the trajectories' `ground_truth_fix_direction` keyword pool. **This is by-design for cost-free real-code-path benchmarking**: it proves DSPy's framework is wired correctly (compile loops execute, optuna integrates, GEPA reflection_lm is set, MCP boundary works) without burning real LLM budget. **It is NOT a substitute for real-LLM validation** — the Stage E trigger is suppressed pending real-LLM bench data. The full report (per-seed numbers, per-category breakdown, ASCII chart, caveats) is at [`docs/10-self-evolution/dspy-validation-report.md`](./dspy-validation-report.md). Linear issue: [QUI-147](https://linear.app/quilin-agent/issue/QUI-147).

**Stage D 验证产出（2026-05-10 round-3，真实 DSPy 代码路径 + DummyLM judge —— 取代 round-2 mocked 数据）** —— 2026-05-10 在 `scripts/bench-real-dspy.py` 中跑真实 DSPy 3.2.1：真 `MIPROv2`（基于 `optuna` 的 Bayesian 指令搜索）与真 `GEPA`（Genetic-Pareto rollouts）的完整 compile loop 都在执行，judge 用 `dspy.utils.DummyLM`（确定性零成本，`QUILIN_OPTIMIZER_JUDGE_MODE=dummy`）。3-seed 池化（n=150）结果：

| Arm | 池化失败率 | 95% CI (Wilson) |
|---|---|---|
| `PromptRewrite`（启发式 baseline）| 82.0% | [75.1%, 87.3%] |
| DSPy + MIPROv2（真，DummyLM）| 100.0% | [97.5%, 100.0%] |
| DSPy + GEPA（真，DummyLM）| 100.0% | [97.5%, 100.0%] |

两个 DSPy arm 都退到全 fail（两条 lift = **−22.0%**），按规则落在 **lift < 10%** 桶，正常会触发 Stage E。**但这次 regression 完全可解释**：`DummyLM` 输出确定性但**没有语义内容**的响应，DSPy compile loop 生成的"优化后" prompt 缺乏 replay 评分器需要的关键字重叠。`PromptRewrite` baseline 之所以赢，是因为它的 per-category 模板是人工写的，恰好命中 trajectory `ground_truth_fix_direction` 的关键字池。**这是无成本测试真实代码路径的有意设计**：证明 DSPy 框架接线正确（compile loop 运行、optuna 集成、GEPA reflection_lm 已配置、MCP 边界工作）而**不烧真实 LLM 配额**。**这不能替代真实 LLM 验证** —— Stage E 触发条件暂缓，等到真实 LLM bench 数据出来再判。完整报告（per-seed 数字、按类别拆分、ASCII 条形图、限制说明）见 [`docs/10-self-evolution/dspy-validation-report.md`](./dspy-validation-report.md)。Linear issue：[QUI-147](https://linear.app/quilin-agent/issue/QUI-147)。

**Implementation evidence (Stage D real-DSPy path, 2026-05-10)** — Reference points for downstream consumers:
