...
> Quilin Agent 的顶层架构概念。当前全局架构入口见 [Core Loop](./README.md)。对话工程作为 `02-context/conversation-engineering/` 子模块，Iter F 解冻。
>
> **小节编号说明**：§二/六/十二/十四 已合并到 [Core Loop](./README.md) 与各组件 `README.md`，编号保留原位以便回溯外链锚点（§三/四/七/十三）。
...
**Quilin 选择（2026-04-30 校准）**：模式 1 为核心 + 模式 2 的 pre/post guardrails 中间件 + 模式 4 的 SQLite checkpoint + progress 文件持久化。AgentMesh 侧 Iter D 只落 `crates/mesh-sdk` stub；P2P 异构互联 runtime 延期到 Iter F Day 0 spike。

---

## 五、Quilin 的 Harness 架构

Agent Loop（< 200 行 TS）+ 12 个激活工程领域（01..11, 13）+ 1 parked 子模块（`02-context/conversation-engineering/`）共同构成 harness。历史六组件（E-T-C-S-L-V）↔ 领域编号的完整映射已收敛到 [Core Loop](./README.md)。各子系统（prompt 组装、记忆注入、工具定义、安全护栏、策略切换、token 预算、可观测性、自进化 scaffold、Semantic Graph 协调）的详细设计分散在 `docs/<component>/README.md`，此处不再重复索引。
...
18 篇行业文献（OpenAI、Anthropic、Martin Fowler、LangChain、Manus、Epsilla、HumanLayer、snowan、Infralovers、ICSE 2026 等）的结论已吸收到当前组件 README。
