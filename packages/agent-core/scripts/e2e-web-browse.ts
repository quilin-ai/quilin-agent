    ...
    const stubbedPage: StagehandLikePage = {
      ...
      async waitForSelector(selector, options) {
        await playwrightPage.waitForSelector(selector, {
          ...
          state: options?.state === "hidden" || options?.state === "detached"
            ? options.state
            : "attached",
        });
      },
...
function makePureStubFactory(result: unknown = { stubbed: true }): StagehandFactory {
  return async (_config) => {
    const stubbedPage: StagehandLikePage = {
      async goto(_url, _opts) {},
      async waitForSelector(_sel, _opts) {},
      async extract(_args) { return result; },
      async observe(_args) { return result; },
      async act(_args) { return result; },
    };
    return {
      async init() {},
      get page() { return stubbedPage; },
      async close() {},
    };
  };
}
...
async function main() {
  ...
  results.push(
    await runCase("TC-01 extract mode returns structured data", async () => {
      ...
      const extractResult = {
        title: "Test Page",
        description: "A minimal SPA fixture for web_browse E2E acceptance tests.",
      };
    ...
    await runCase(
      ...
      async () => {
        ...
        const ok =
          ...
          confirmCalls[0]!.tool === "web_browse" &&
          confirmCalls[0]!.riskLevel === "medium";
  ...
  results.push(
    await runCase(
      ...
      async () => {
        ...
        const ok = !result.isError && payload.mode === "act" && confirmCalls.length === 0;
  ...
  results.push(
    await runCase(
      ...
      async () => {
        ...
        // Custom factory: goto() triggers the #load click so the 500ms timer
        // starts, then the tool's built-in waitForSelector waits for #price.
        const patchrightMod = patchright!;
        const waitForFactory: StagehandFactory = async (_config) => {
          const browser = await patchrightMod.chromium.launch({ headless: true });
          const context = await browser.newContext();
          const playwrightPage = await context.newPage();
          ...
          const stubbedPage: StagehandLikePage = {
            async goto(url, options) {
              await playwrightPage.goto(url, { timeout: options?.timeout ?? 60_000 });
              // Click #load to start the 500ms timer that injects #price
              await playwrightPage.click("#load");
            },
            async waitForSelector(selector, options) {
              // Real Patchright wait — will block until #price appears
              await playwrightPage.waitForSelector(selector, {
                timeout: options?.timeout ?? 60_000,
                state: "attached",
              });
            ...
            async extract(_args) {
              // Read the actual DOM value to prove the element appeared
              const text = await playwrightPage
                .locator("#price")
                .textContent()
                .catch(() => null);
              return { price: text };
            },
            async observe(_args) { return []; },
            async act(_args) { return {}; },
          };
  ...
  // -------------------------------------------------------------------------
  // TC-08: URL with userinfo — expected rejection, currently a DEFECT
  // -------------------------------------------------------------------------
  results.push(
    await runCase("TC-08 URL with userinfo (user:pass@host) is rejected", async () => {
      ...
      if (result.isError && /userinfo|credential|password|user:/i.test(payload.error ?? "")) {
        return {
          status: "pass",
      ...
      // Defect: tool accepted a URL with embedded credentials
      return {
        status: "fail",
        notes: `DEFECT: userinfo URL was not rejected; actual URL used: ${payload.url ?? "(unknown)"}. web-browse.ts only checks protocol, not userinfo — needs a guard: if (parsedUrl.username || parsedUrl.password) reject.`,
        observed: payload,
      };
    }),
  );
  ...
  // -------------------------------------------------------------------------
  results.push(
    await runCase("TC-09 no Chromium process leak after tool closes", async () => {
      if (!patchrightAvailable) {
        return { status: "skip", notes: "patchright not available — skipping leak check" };
      }
      ...
      return {
        ...
        observed: { beforeCount, afterCount, delta, execIsError: execResult.isError },
      };
    }),
  );
