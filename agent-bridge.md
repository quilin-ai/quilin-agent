...
**核心原则**：subagent 是用来**并行**的，不是用来"代办"的。判断时问自己：派出 subagent 后，主线程有别的工作可推进吗？

- **能并行**（主线程同时还有 plan / review / 其他独立任务可做）→ 长任务（> 5 分钟）起 subagent，让主线程继续推
- **不能并行**（只有这一个任务、主线程派完会闲等通知）→ **直接主线程做**。绕 worktree → cherry-pick 仪式只换来"主线程闲等 + worktree-cwd bug 风险（已栽过 2 次）"，没有真正的并行收益

**3 分钟自检**：主线程任务执行到约 3 分钟仍未完成、预计还需继续明显工作时，做一次自检：
- 快收尾 → 主线程吃完
- 还要继续扩展且**有其他任务在飞** → 升级为 subagent
- 还要继续扩展但**没别的任务** → 留在主线程吃完
...
**已知 worktree-cwd 陷阱（避免理由之一）**：subagent 的 `cd` 在 Bash 调用之间不持久，多次发生 subagent 误写主仓库工作区然后自纠的情况。主线程做不绕这个坑。
